...
L_file = 'bt.l3d';
line_matches_file = 'bt.NVIEW-LINES';
img1_nr = 0;
N_IMGS = 11;
img_part_name = 'bt.';
img_ext = '.pgm';
...
% 			matches{i} = NaN;
	...
	matches = reshape(matches, N_IMGS, 27);
	matches = matches';
	...
	C_GT = C_GT * T_FORM_2D;
    P_estim = C_GT * R_GT_VGG * [eye(3) -T_GT_VGG];
	...
	R_GT =  R_GT_VGG;
	T_GT = - R_GT_VGG*T_GT_VGG;
    T_motion = [R_GT T_GT];
%     camfile = ['cameras' sprintf('%03d', img_nr) '.txt'];
%     dlmwrite(camfile,T_motion, ' ');
%     X_W_GT = [P_GT; 0 0 0 1 ]* X_W;
	 X_W_ = [R_GT  T_GT] * X_W;
	%% Read 2D lines
...
%     errrorr = proj(x_img) - proj(X_W_GT )
...
%     errrrorrrr = proj(x_c) - proj([R_RPnL R_RPnL*T_RPnL] * X_W)
    ...
    cam = eye(3);
    t_estim = rand(3,1);
%     [R_BA, T_BA]= linePoseEstim_3e(X_W, X_W_,cam, R_estim,  t_estim);
    
    [R_BA, T_BA]= linePoseEstim_3e(X_W, x_c,cam, R_RPnL, T_RPnL);
    disp('Yucao [R|T]');
%     T_BA = R_BA \ T_BA;
