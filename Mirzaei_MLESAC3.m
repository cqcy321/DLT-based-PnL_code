function [ R, T ] = Mirzaei_MLESAC3( X_W, x_c, probab, max_iter, Ep_th)
%MIRZAEI_MLESAC3 Camera pose estimation from line correspondences using
% the Mirzaei method plugged into a MLESAC loop.
%
...
%   The 3D camera coordinate system is right-handed: X-right, Y-up, Z-back.
	...
	is_inlier = MLESAC(X_W, x_c, @Mirzaei_wrapper, MIN_LINES, probab, max_iter, Ep_th);
	...
	if (sum(is_inlier) < MIN_LINES)
		...
		% enough inliers
		[R, T] = Mirzaei_wrapper(X_W_inliers, x_c_inliers);
	end
