function [ R, T ] = DLT_Combined_Lines_AOR( X_W, x_c, reproj_err_th, focal )
%DLT_COMBINED_LINES_AOR Camera pose estimation from line correspondences using
% the DLT-Combined-Lines method with Algebraic Outlier Rejection module.
%
...
%   The 3D camera coordinate system is right-handed: X-right, Y-up, Z-back.
	...
	% Do NOT normalize point and line measurement matrices here
	...
	while(true)
		...
		w4 = [is_inlier; is_inlier; is_inlier; is_inlier];
		WM = bsxfun(@times, M, w4); % WM = W * M = diag(w4) * M;
		...
		% Vectorized combined projection matrix as the nullspace of the measurement 
		% matrix from the last right singular vector
		p_e = V_WM(:,end);

		residual = M * p_e;
		residual = reshape(residual, N_LINES, 4);
	...
	[R, T] = DLT_Combined_Lines(X_W(:, is_inlier), x_c(:, is_inlier));
	return;
end
